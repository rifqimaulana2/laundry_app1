...
@section('title', 'Daftar Layanan Kiloan')

@section('content')
<div class="max-w-5xl mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Daftar Layanan Kiloan</h1>
        <a href="{{ route('superadmin.layanan-kiloan.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2">
                ...
                @forelse ($layananKiloans as $layanan)
                    <tr class="border-t hover:bg-gray-50">
                        ...
                        <td class="p-4 flex gap-2">
                            <a href="{{ route('superadmin.layanan-kiloan.edit', $layanan) }}" class="px-3 py-1 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 flex items-center gap-2">
                                <i data-lucide="edit" class="w-4 h-4"></i> Edit
                            </a>
                            <form action="{{ route('superadmin.layanan-kiloan.destroy', $layanan) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
