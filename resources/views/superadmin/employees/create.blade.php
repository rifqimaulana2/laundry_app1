...
<div class="max-w-3xl mx-auto px-4 py-8">
    ...
    <form action="{{ route('superadmin.employees.store') }}" method="POST" class="bg-white p-6 rounded-2xl shadow-lg">
        @csrf
        <div class="mb-5">
        ...
        <div class="mb-5">
            ...
            <select name="mitra_id" class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('mitra_id') border-red-500 @enderror" required>
                ...
                @foreach ($mitras as $mitra)
                    <option value="{{ $mitra->id }}">{{ $mitra->nama_toko }}</option>
                @endforeach
            </select>
            @error('mitra_id')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        ...
        <div class="mb-5">
            ...
            <input type="text" name="no_telepon" class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('no_telepon') border-red-500 @enderror" value="{{ old('no_telepon') }}" required>
            @error('no_telepon')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
